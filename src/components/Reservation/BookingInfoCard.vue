<template>
  <v-card class="card booking-info" dark width="100%" max-width="330px">
    <v-row no-gutters>
      <v-flex xs12 md6>
        <h2>
          {{ resort.title }}
      ...
      <v-flex class="normalText">
        <v-icon class="icon">event</v-icon> {{ cardDate(dateOne) }}
        <v-icon class="icon">arrow_right_alt</v-icon> {{ cardDate(checkOut) }}
      </v-flex>
      <v-flex xs12>
        <v-divider class="divider"></v-divider>
      </v-flex>
